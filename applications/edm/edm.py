...
def dataForFreq(frequency: float, time: float = None):
...
def play(frequency: float, time: float):
    frames = dataForFreq(frequency, time)
    stream = piano.open(format=FORMAT, channels=CHANNELS, rate=RATE, output=True)
    stream.write(frames)
    stream.stop_stream()
    stream.close()
...
def playNote(a, b):
    global expression
    char = mapToTile(a, b).rstrip()
    if char == "~":
        print(colored("Wrong coordinates!", "red"))
        return
    print(colored(f"NOTE: {char}", "blue"))
    play(pitchMap[char], TIME)
